...
public class QueryShipment {
    private static final String QUERY_SHIPMENT_CARRIER = 
            "SELECT * FROM ENVIO , PUJA WHERE PUJA.EMAIL = ? AND ENVIO.NROENVIO = PUJA.NROENVIO";
    
    
    private static final String QUERY_SHIPMENT_CLIENT = "SELECT * FROM ENVIO WHERE EMAIL = ?";
    ...
    public static List<Shipment> getShipmentList(String email) throws SQLException{
        ...
        if (QueryUser.getRole(email) == Role.TRANSPORTISTA){
            return getShipmentList(email,QUERY_SHIPMENT_CARRIER);
        } else {
            return getShipmentList(email, QUERY_SHIPMENT_CLIENT);
        }
    }
